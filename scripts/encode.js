...
/**
 *
 * @param {{x:number, y:number, w:number, h:number, is_show_button:boolean, is_show_image:boolean, is_show_text:boolean, text:string, image:string }[]} ui_elements
...
export function encode(ui_elements, form_size) {
	...
	for (let ui_element of ui_elements) {
		count += 1;
		const { x, y, w, h, is_show_button, is_show_image, is_show_text, text, texture } = ui_element;
		...
		let data2 = text;
		console.warn(JSON.stringify({ x: offset_x_inc + x - count, y: offset_y_inc + y - 1 }));
		let data3_temp = [`${w}`, `${h}`, `${offset_x_inc + x - count}`, `${offset_y_inc + y - 1}`];
